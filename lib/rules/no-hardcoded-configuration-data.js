module.exports = {
  ...
  create(context) {
    ...
    const forbid =
      (context.options[0] && context.options[0].forbidContaining) ||
      DEFAULT_FORBID;
    const exclude =
      (context.options[0] && context.options[0].excludeContaining) ||
      DEFAULT_EXCLUDE;
    ...
    function inspect(regex, node, value) {
      ...
      if (
        exclude.some((name) => value.toLowerCase().includes(name.toLowerCase()))
      )
        return;
      ...
      if (REGEX[regex].test(value)) {
        return context.report({
          node,
          message: "Do not use hardcoded configuration data",
        });
      }
    }
